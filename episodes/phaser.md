...
属性 type 可以是 Phaser.CANVAS，或者 Phaser.WEBGL，或者 Phaser.AUTO。这是你要给你的游戏指定渲染环境。推荐值是 Phaser.AUTO ，它会自动尝试使用 WebGL，如果浏览器或设备不支持，它将回退为 Canvas 。然后画布的大小设置为800x600像素。然后给场景设置三个函数。
...
可以从 [gameart2d](https://www.gameart2d.com/free-graveyard-platformer-tileset.html) 下载一些游戏中需要的图片。解压后放到项目的 `assets` 文件夹下。这个比较简单，不演示了。

显示图片需要用到 preload 和 create 这两个函数。preload 顾名思义就是在程序启动之前先去加载一下资源，例如音频或者图片。而 create 就是要构建游戏场景的画面了，所以到底图片如何显示，显示到什么位置，都是在 create 函数中去执行的。注意，这两个函数都是在游戏启动时会被环境自动执行的。
...
这就要到 create 函数中去写了，三个参数，前两个是图片中心的横纵坐标，最后一个是图片的名字。浏览器中可以看到，背景图片放到了画布的中心，因为整个画布是800乘以600的。400和300刚好是中心位置。
...
一个游戏人物是要处于一个物理场景下的。到配置部分添加一个名为 arcade 的物理状态。重力设置 y 方向，那么人物就会被来自下方的引力，很显然，这里也允许配置让引力来自上方或者水平方向。debug 这一项的作用稍后会看到。

Arcade 这个物理场景下有两种类型的物体，一种是静态的，另外一种是动态的。静态的就是不受外力影响的，总是静静的原地不动。动态的物体，可以有速度，会收到重力影响，可以跟其他物体发生碰撞。
...
添加一个全局变量来存放静态物体。到 preload 函数中，加载一个合适的图片，到 create 函数中，先来添加一个静态物体，设置合适的坐标摆放一下。
...
但是现在隐含了一个问题。把 debug 模式打开。可以看到虽然图片缩小了，但是原有的区域，还是被占据者，所以如果未来有动态的人物站在上面，那么人物就是悬空的。
...
需要做的是到 create 函数中添加这一句，这样，两个物体之间就会碰撞，而不是直接忽视。


但是如何让这个小人物动起来呢？这里的主角就是 update 函数了。一个游戏之所以人眼看起来图像很平滑，就是因为场景会在一秒之内更新很多次。update 函数会在游戏启动后，每秒被执行60次。
...
create 函数里面添加这一条，一般通过 cursors 获取到键盘状态。
...
update 中的函数很直观。如果做方向键被按下，那就让人物获得一向左的速度，如果右键按下那就速度向右，抬起左右键的时候，让速度归零。
...
`player.body.touching.down` 保证了，只有当人物的下方是跟其他物体接触的时候，按方向键才会有向上的速度。具体的介绍可以参考官网的 [API 文档](https://newdocs.phaser.io/docs/3.55.2/Phaser.Physics.Arcade.Body#touching)



## 总结

把 `debug` 改成 false ，这样浏览器中可以看到，我们可以正常控制人物了。